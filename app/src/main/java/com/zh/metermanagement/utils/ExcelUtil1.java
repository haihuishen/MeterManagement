...
public class ExcelUtil1 {
    ...
    public static List<MeterBean1> readMeterInfoExcel(String path) {
        ...
        try {
            ...
            for (int i = 2; i < rows; ++i) {
                ...
                bean.setUserNumber(sheet.getCell(0, i).getContents());                   //用户编号
                bean.setUserName(sheet.getCell(1, i).getContents());                     //用户名称
                bean.setUserAddr(sheet.getCell(2, i).getContents());                     //用户地址
                bean.setOldAssetNumbers(sheet.getCell(4, i).getContents());              //旧表资产编号(导入的)
                bean.setMeasurementPointNumber(sheet.getCell(5, i).getContents());       //计量点编号
                bean.setPowerSupplyBureau(sheet.getCell(8, i).getContents());            //供电局(供电单位)
                bean.setTheMeteringSection(sheet.getCell(13, i).getContents());          //抄表区段
                bean.setCourts(sheet.getCell(14, i).getContents());                      //台区
                bean.setMeasuringPointAddress(sheet.getCell(25, i).getContents());    //计量点地址
    ...
    public static List<MeterPhoneBean> readMeterPhoneExcel(String path) {
        ...
        try {
            ...
            for (int i = 2; i < rows; ++i) {
                ...
                bean.setUserNumber(sheet.getCell(0, i).getContents());                   //用户编号
                bean.setUserPhone(sheet.getCell(10, i).getContents());                     //用户电话
                bean.setTheMeteringSection(sheet.getCell(11, i).getContents());            //抄表区段
