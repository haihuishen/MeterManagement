...
public class BeepManager {
	...
	private void initial() {
		...
		loadId1 = mSoundPool.load(mContext, ResourceUtil.getRawResIDByName(mContext, "error"), 1);
		loadId2 = mSoundPool.load(mContext, ResourceUtil.getRawResIDByName(mContext, "successful"), 1);
		// initialVibrator
	...
	public void playSuccessful() {
		...
		if (playBeep) {
			...
			// 参数5：是否循环播放，0只播放一次(0 = no loop, -1 = loop forever)
			// 参数6：特效音乐播放的速度，1F为正常播放，范围 0.5 到 2.0
			mSoundPool.play(loadId2, 0.5f, 0.5f, 1, 0, 1f);
		}
		// vibrate
		if (vibrate) {
			mVibrator.vibrate(VIBRATE_DURATION);
